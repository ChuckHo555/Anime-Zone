...
const NoteCard: React.FC<AnimeCardProps> = ({
  ...
  // Fetch saved review and score when the component mounts
  useEffect(() => {
    const fetchSavedData = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setReview(data.note || ""); // Load saved note
          setScore(data.userRating || 0); // Load saved score
    ...
    try {
      const res = await fetch(`/api/getNotes`, {
        ...
        body: JSON.stringify({
          userId,
          animeId: anime.id,
          note: review || "", // Always send the current review
          userRating: score || 0, // Always send the current score
        }),
      });
  ...
  const handleSaveReview = () => {
    saveChanges(); // Save the review and score
  };
  ...
  return (
    <div className="flex w-full p-4 bg-gray-800 rounded-lg shadow-md items-stretch gap-4">
      ...
      <div className="flex flex-col justify-between gap-2">
        ...
        <div className="flex flex-col justify-center items-center bg-purple-700 h-28 w-28 rounded-md">
